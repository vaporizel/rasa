...
class StatefulTurn(DefinedTurn):
    ...
    Why is the first turn empty?

        In addition to creating turns as described above, we prepend an initial turn
        that is empty: It captures no events, contains an empty state, and is
        attributed to the bot. This special turn is used by our rule policies.
    ...
    def __repr__(self) -> Text:
        return f"{self.__class__.__name__}({self.actor}):{self.events}:{self.state}"

    def __str__(self) -> Text:
        return f"{self.__class__.__name__}({self.actor}):{self.state}"
    ...
    def parse(
        ...
        Args:
            ...
            rule_only_data: Slots and loops, which only occur in rules but not in
              stories.
            from_applied_events: generate turns from applied instead of raw events
        ...
        events = tracker.applied_events() if from_applied_events else tracker.events
