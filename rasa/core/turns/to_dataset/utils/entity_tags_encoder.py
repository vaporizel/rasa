...
class EntityTagsEncoder:
    def __init__(self, domain: Domain, bilou_tagging: bool = False) -> None:
        ...
        if bilou_tagging:
            tag_id_index_mapping = {
                ...
                for tag, idx_1 in multihot_encoder.dimension_name_to_index.items()
                for idx_2, prefix in enumerate(BILOU_PREFIXES)
            }
        else:
            tag_id_index_mapping = {
                tag: idx + 1  # +1 to keep 0 for the NO_ENTITY_TAG
                for tag, idx in multihot_encoder.dimension_name_to_index.items()
            }
    ...
    def encode(
        self, text_tokens: Optional[List[Token]], entities: Optional[Dict[Text, Any]],
